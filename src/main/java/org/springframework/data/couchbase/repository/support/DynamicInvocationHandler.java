...
public class DynamicInvocationHandler<T1> implements InvocationHandler {
	...
	final CouchbaseEntityInformation<T1, String> entityInformation;
	final CouchbaseTemplate executableTemplate;
	...
	public DynamicInvocationHandler(T1 target, CommonOptions<?> options, String collection, String scope) {
		...
		if (target instanceof CouchbaseRepository) {
			executableTemplate = (CouchbaseTemplate) ((CouchbaseRepository) target).getOperations();
			reactiveTemplate = null;
		} else if (target instanceof ReactiveCouchbaseRepository) {
			executableTemplate = null;
		...
		this.entityInformation = target instanceof CouchbaseRepository
				? ((CouchbaseRepository<T1, String>) target).getEntityInformation()
				: ((ReactiveCouchbaseRepository<T1, String>) target).getEntityInformation();
	...
	private void setThreadLocal() {
		if (executableTemplate != null) {
			executableTemplate.setThrdLocalArgs(new PseudoArgs(this.options, this.scope, this.collection));
		} else {
			reactiveTemplate.setThrdLocalArgs(new PseudoArgs(this.options, this.scope, this.collection));
		}
	}

	private void clearThreadLocal() {
		if (executableTemplate != null) {
			executableTemplate.setThrdLocalArgs(null);
		} else {
			reactiveTemplate.setThrdLocalArgs(null);
		}
	}
}
