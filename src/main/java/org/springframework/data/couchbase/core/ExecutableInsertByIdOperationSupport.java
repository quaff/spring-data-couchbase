...
public class ExecutableInsertByIdOperationSupport implements ExecutableInsertByIdOperation {
	...
	static class ExecutableInsertByIdSupport<T> implements ExecutableInsertById<T> {
		...
		ExecutableInsertByIdSupport(final CouchbaseTemplate template, final Class<T> domainType, final String scope, final String collection, final InsertOptions options,
			...
			this.expiry = expiry;
			this.reactiveSupport = new ReactiveInsertByIdSupport<>(template.reactive(), domainType, scope, collection, options, persistTo,
		...
		@Override
		public TerminatingInsertById<T> withOptions(InsertOptions options) {
			return new ExecutableInsertByIdSupport<>(template, domainType, scope, collection, options, persistTo, replicateTo,
					durabilityLevel, expiry);
		}

		@Override
		public InsertByIdWithCollection<T> inScope(String scope) {
			return new ExecutableInsertByIdSupport<>(template, domainType, scope, collection, options, persistTo, replicateTo,
					durabilityLevel, expiry);
		}
