...
public class ExecutableFindByQueryOperationSupport implements ExecutableFindByQueryOperation {
	...
	static class ExecutableFindByQuerySupport<T> implements ExecutableFindByQuery<T> {
		...
		@Override
		@Deprecated
		public FindByQueryWithScope<T> consistentWith(final QueryScanConsistency scanConsistency) {
		...
		public <R> FindByQueryWithConsistency<R> as(final Class<R> resturnType) {
			...
			return new ExecutableFindByQuerySupport<>(template, domainType, resturnType, query, scanConsistency, scope,
					collection, options, distinctFields);
		}
		...
		@Override
		public TerminatingFindByQuery<T> withOptions(QueryOptions options) {
			return new ExecutableFindByQuerySupport<>(template, domainType, returnType, query, scanConsistency, scope,
					collection, options, distinctFields);
		}

		@Override
		public FindByQueryInCollection<T> inScope(String scope) {
			return new ExecutableFindByQuerySupport<>(template, domainType, returnType, query, scanConsistency, scope,
					collection, options, distinctFields);
		}
