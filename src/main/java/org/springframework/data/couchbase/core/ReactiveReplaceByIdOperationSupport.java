...
public class ReactiveReplaceByIdOperationSupport implements ReactiveReplaceByIdOperation {
	...
	static class ReactiveReplaceByIdSupport<T> implements ReactiveReplaceById<T> {
		...
		public Mono<T> one(T object) {
			...
			LOG.info("statement: {} scope: {} collection: {}", "replaceById", pArgs.getScope(), pArgs.getCollection());
			return Mono.just(object).flatMap(o -> {
				...
				return template.getCouchbaseClientFactory().withScope(pArgs.getScope()).getCollection(pArgs.getCollection())
						.reactive().replace(converted.getId(), converted.export(), buildReplaceOptions(o, converted))
